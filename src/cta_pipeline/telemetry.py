...
class TelemetryPipeline:
    def __init__(self, db, vehicle_client=None, trip_client=None, clock=None,
                 ...
                 stationary_seconds=600, gap_seconds=900, explainer=None):
        self.db=db; key=os.getenv("CTA_API_KEY")
        self.vehicle_client=vehicle_client or GTFSRealtimeClient("VehiclePositions",key)
        self.trip_client=trip_client or GTFSRealtimeClient("TripUpdates",key)
    ...
    def _persist(self,vf,tf,vehicles,predictions,anomalies,stamp):
        ...
        with self.db.connect() as con:
            ...
            cur=con.execute("insert into telemetry_runs(started_at,status) values(?,?)",(stamp,"running")); run=cur.lastrowid
            for name,feed in (("vehicle_positions",vf),("trip_updates",tf)):
            ...
            for r in vehicles:
                ...
                fields=("route_id","direction_id","latitude","longitude","stop_id","current_status","vehicle_timestamp")
                if old is None or any(old[k]!=r[k] for k in fields):
                    con.execute("insert into vehicle_observations(run_id,vehicle_id,route_id,direction_id,latitude,longitude,stop_id,current_status,vehicle_timestamp,observed_at) values(?,?,?,?,?,?,?,?,?,?)",(run,r["vehicle_id"],r["route_id"],r["direction_id"],r["latitude"],r["longitude"],r["stop_id"],r["current_status"],r["vehicle_timestamp"],stamp))
            ...
            self._cap_runs(con)
        return {"run_id":run,"status":"success","vehicles":len(vehicles),"predictions":len(predictions),"snapshots_created":created,"new_anomalies":new_anomalies,"vehicle_feed_timestamp":vf["header"]["timestamp"],"trip_feed_timestamp":tf["header"]["timestamp"]}

    def _failure(self,stamp,exc):
        try:
            with self.db.connect() as con:
                con.execute("insert into telemetry_runs(started_at,finished_at,status,error) values(?,?,?,?)",(stamp,stamp,"failed",f"{type(exc).__name__}: telemetry cycle failed"[:MAX_ERROR]))
                self._cap_runs(con)
